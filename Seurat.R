...
p=DimPlot(obj.in, label=F, reduction = "umap", group.by= "project", pt.size=1.1)
save_plot(p, paste("Seurat", "Dimplot_umap_condition", sep="/"), fig.width=10, fig.height=6)


##############
deResTT = makeDEResults(obj.in, group.by="seurat_clusters", assay="RNA", test="t")
...
p=DimPlot(obj.in, reduction = "umap", label=T, pt.size=1.1, group.by= "CellType") 
