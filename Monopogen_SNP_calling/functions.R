...
compareCellsByCluster = function(inobj, cellsID1, cellsID2, suffix1, suffix2, group.by=NULL, assay="RNA", test="LR", outfolder="./", fcCutoff=0.25, all=FALSE, heatmap.plot=FALSE, heatmap.pcutoff=0.05, heatmap.addgenes=NULL)
{
  if (is.null(group.by))
  {
    clusterIDs = as.character(sort(unique(Idents(inobj))))  
...
makeVolcanos = function(loMG, titlePrefix, outname, restrict_labels=NULL, turnExpression=F, colors = list(neg=list(sig="#448CCA", nosig="#B4D1E9"), pos=list(sig="#F47B78", nosig="#FACAC9")), FCcutoff=0.5, pCutoff = 0.05, highlightGene=NULL, width = 12, height = 7)
{
  ...
  for (cName in names(loMG))
  {
    ...
    # --- ALWAYS start with automatic labels ---
   selDF = indf[indf$p_val_adj < pCutoff, ] %>%
  arrange(p_val_adj) %>%
  head(40)

  plotlabels = selDF$gene

  # --- optional: override with restrict_labels ---
  ...
  # --- ALWAYS add highlight genes on top ---
  ...
  if (length(possibleGenes) > 0)
  {
    ...
    plotlabels = unique(c(possibleGenes, plotlabels))
  }
}
    ...
    print(filename)
    png(filename=filename,width = width*100, height = height*100)
    ...
    print(filename)
    svglite(file = filename, width = width, height = height)
    p=EnhancedVolcano(indf,
      lab = indf$gene,
      selectLab=plotlabels,
      x = 'avg_log2FC',
